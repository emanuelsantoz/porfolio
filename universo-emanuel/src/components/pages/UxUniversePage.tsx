...
export function UxUniversePage() {
  ...
  useEffect(() => {
    setPersona("ux-ui");
    const timer = window.setTimeout(() => setEntering(false), shouldReduceMotion ? 0 : 1250);
    return () => { window.clearTimeout(timer); setPersona("fullstack"); };
  }, [setPersona, shouldReduceMotion]);

  return <main className="min-h-screen bg-[#1E1E1E] text-[#F5F5F5]">
    <AnimatePresence>{entering && <motion.div initial={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: shouldReduceMotion ? 0 : 0.35 }} className="fixed inset-0 z-[100] flex items-center justify-center overflow-hidden bg-[#07111f] px-6"><NexusField tone="dark" className="absolute w-[30rem] max-w-[90vw] opacity-65" /><motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: shouldReduceMotion ? 0 : 0.55 }} className="relative max-w-xl text-center"><p className="font-mono text-xs uppercase tracking-[0.24em] text-[#79aef4]">ES/0 → UX/UI</p><p className="mt-6 font-[family-name:var(--font-display)] text-4xl font-medium leading-tight tracking-[-0.05em] sm:text-6xl">Uma conexão começa a ganhar interface.</p><div className="mx-auto mt-8 h-px w-44 overflow-hidden bg-white/20"><motion.div initial={{ x: "-100%" }} animate={{ x: "100%" }} transition={{ duration: shouldReduceMotion ? 0 : 1, ease: "easeInOut" }} className="h-full bg-[#F24E1E]" /></div></motion.div></motion.div>}</AnimatePresence>
    ...
    <section className="relative overflow-hidden border-b border-white/10 px-6 py-20 lg:px-10 lg:py-28" style={{ backgroundImage: "radial-gradient(circle at 80% 5%, rgba(242,78,30,.24), transparent 30%), linear-gradient(#ffffff08 1px, transparent 1px), linear-gradient(90deg, #ffffff08 1px, transparent 1px)", backgroundSize: "auto, 40px 40px, 40px 40px" }}><NexusField tone="ux" className="absolute -right-20 -top-24 w-[34rem] opacity-45" /><div className="relative mx-auto max-w-7xl"><p className="font-mono text-xs uppercase tracking-[0.22em] text-[#F24E1E]">Experiência e interface</p><h1 className="mt-6 max-w-5xl font-[family-name:var(--font-display)] text-5xl font-medium leading-[.98] tracking-[-0.06em] sm:text-7xl">Como isso pode fazer sentido para quem vai usar?</h1><p className="mt-8 max-w-2xl text-lg leading-8 text-white/68">Este não é um portfólio de telas prontas. É um espaço para mostrar como uma pergunta passa por contexto, jornada e decisão antes de virar interface.</p><a href="#processo" className="mt-10 inline-flex items-center gap-2 rounded-full bg-[#F24E1E] px-5 py-3 text-sm font-semibold text-white transition hover:bg-[#ff6538]">Ver como penso <ArrowUpRight size={17} /></a></div></section>
